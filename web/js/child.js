(() => {
  ...
  let currentStep = 0, activePadTarget = "name", kanaPage = "basic", transferInfo = { enabled:false, baseUrl:"", apiBaseUrl:"" };
  ...
  async function openCardDialog(){const dialog=$("#cardDialog"),box=$("#cardQrBox"),status=$("#cardQrStatus"),link=$("#cardShareLink");dialog.showModal();box.innerHTML="<span>しゃしんを つくっています…</span>";link.hidden=true;if(!state.name.trim()){status.textContent="なまえを いれると おくれるよ。";box.innerHTML="<span>なまえを いれてね</span>";return;}try{await drawCard();}catch(_){box.innerHTML="<span>画像をつくれませんでした</span>";status.textContent="もういちどためしてね。";return;}if(!transferInfo.enabled&&!(await checkTransferServer())){status.textContent="QR機能用サーバーに接続できません。このパソコンへの保存はできます。";box.innerHTML="<span>QR機能用サーバーが必要です</span>";return;}try{const dataUrl=$("#cardCanvas").toDataURL("image/png"),response=await apiFetch("/api/cards",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:dataUrl,filename:`${safeFilename(state.name)}-card.png`,title:`${state.name}のカード`})}),data=await response.json();if(!response.ok)throw new Error();const img=new Image();img.alt="カードをうけとるQRコード";img.onload=()=>box.replaceChildren(img);img.src=apiUrl(`/api/qr?text=${encodeURIComponent(data.cardUrl)}&t=${Date.now()}`);link.href=data.cardUrl;link.hidden=false;status.textContent="スマホで よみこんで、プレビューとおなじ写真を ほぞんしてね。";}catch(_){box.innerHTML="<span>QRをつくれませんでした</span>";status.textContent="このパソコンへの画像保存はできます。";}}
