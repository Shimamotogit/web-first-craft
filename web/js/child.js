(() => {
  ...
  function init() {
    restoreControls(); renderKana(); bind(); applyInputMode(false); renderAll(); setKanaVisibility(state.kanaVisible !== false, false); checkLan();
  }
  ...
  async function openPhonePhotoTransfer(){if(!await ensureLan())return;closePhotoPolling();const dialog=$("#photoDialog"),box=$("#photoQrBox"),status=$("#photoQrStatus"),link=$("#photoShareLink");box.innerHTML="<span>QRをつくっています…</span>";link.hidden=true;status.textContent="アップロード用QRをつくっています…";dialog.showModal();try{const response=await fetch("/api/photo-sessions",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}),data=await response.json();if(!response.ok)throw new Error();activePhotoToken=data.token;const img=new Image();img.alt="しゃしんをおくるQRコード";img.onload=()=>box.replaceChildren(img);img.src=`/api/qr?text=${encodeURIComponent(data.uploadUrl)}&t=${Date.now()}`;link.href=data.uploadUrl;link.hidden=false;status.textContent="スマホでQRをよんで、しゃしんをえらんで『パソコンへ送る』をおしてね。";pollPhotoSession();}catch(_){box.innerHTML="<span>QRをつくれませんでした</span>";status.textContent="LANサーバーに接続できません。インストール時に表示された「このPC」または「同じLAN」のURLから開いているか確認してね。";}}
  ...
  async function checkLan(){try{const response=await fetch("/api/config",{cache:"no-store"});if(!response.ok)throw new Error();const data=await response.json();lanInfo={enabled:Boolean(data.enabled),baseUrl:String(data.baseUrl||"")};}catch(_){lanInfo={enabled:false,baseUrl:""};}return lanInfo.enabled;}async function ensureLan(){if(lanInfo.enabled)return true;if(await checkLan())return true;showToast("QRはLANサーバーから開くと つかえるよ。インストール時に表示されたURLから開いてね");return false;}
