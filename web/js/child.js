(() => {
  ...
  function buildKidSvg(){
    ...
    const defs=`<defs><clipPath id="photoClip" clipPathUnits="objectBoundingBox">${photoClipShape(state.frame)}</clipPath><clipPath id="decorSafeZone"><rect x="0" y="0" width="900" height="88"/><rect x="0" y="1112" width="900" height="88"/><rect x="0" y="88" width="52" height="1024"/><rect x="848" y="88" width="52" height="1024"/></clipPath><pattern id="checks" width="56" height="56" patternUnits="userSpaceOnUse"><rect width="28" height="28" fill="${p.accent2}" opacity=".25"/><rect x="28" y="28" width="28" height="28" fill="${p.accent2}" opacity=".25"/></pattern></defs>`;
    ...
    // 背景もよう → 本文 → 外周かざり。かざりは安全な外周帯にclipして、写真や文字を隠さない。
  ...
  function decorSvg(decor,p){
    const open=`<g data-decor-zone="edge" clip-path="url(#decorSafeZone)">`,close=`</g>`;
    if(decor==="stickers")return `${open}<g font-size="56"><text x="14" y="70" transform="rotate(-10 14 70)">🌈</text><text x="838" y="68" transform="rotate(9 838 68)">⭐</text><text x="12" y="1188" transform="rotate(8 12 1188)">💛</text><text x="838" y="1180" transform="rotate(-10 838 1180)">✨</text></g><g fill="none" stroke="${p.ink}" stroke-width="5" opacity=".58"><circle cx="26" cy="350" r="24"/><circle cx="874" cy="760" r="22"/><path d="M20 570 q25-28 50 0"/><path d="M830 980 q25 28 50 0"/></g>${close}`;
    if(decor==="scrap")return `${open}<g opacity=".96"><rect x="42" y="18" width="180" height="42" fill="#fff1a8" stroke="${p.ink}" stroke-width="2" transform="rotate(-5 42 18)"/><rect x="680" y="24" width="170" height="43" fill="#d9f0dc" stroke="${p.ink}" stroke-width="2" transform="rotate(5 680 24)"/><rect x="25" y="330" width="42" height="170" fill="#ffd3df" stroke="${p.ink}" stroke-width="2" transform="rotate(-4 25 330)"/><rect x="836" y="710" width="42" height="180" fill="#c7eafa" stroke="${p.ink}" stroke-width="2" transform="rotate(4 836 710)"/><rect x="115" y="1145" width="190" height="38" fill="#ffe5a5" stroke="${p.ink}" stroke-width="2" transform="rotate(3 115 1145)"/><rect x="610" y="1140" width="165" height="40" fill="#d8efcf" stroke="${p.ink}" stroke-width="2" transform="rotate(-4 610 1140)"/></g>${close}`;
    if(decor==="festival")return `${open}<g stroke="${p.ink}" stroke-width="3"><path d="M0 34 Q450 88 900 28" fill="none"/><path d="M70 42 l42 5-24 39z" fill="#ff7b78"/><path d="M175 53 l42 4-23 39z" fill="#ffd45a"/><path d="M280 61 l42 2-21 39z" fill="#75c9e8"/><path d="M385 65 l42 0-20 39z" fill="#8fd18b"/><path d="M490 63 l42-2-18 39z" fill="#ff9fc1"/><path d="M595 55 l42-4-18 39z" fill="#ffd45a"/><path d="M700 44 l42-6-18 39z" fill="#75c9e8"/></g><g fill="${p.ink}" opacity=".48"><circle cx="24" cy="245" r="7"/><circle cx="872" cy="300" r="8"/><circle cx="28" cy="620" r="6"/><circle cx="873" cy="680" r="7"/><circle cx="26" cy="1000" r="8"/><circle cx="872" cy="1035" r="6"/></g><g fill="#ff7b78" opacity=".8"><rect x="170" y="1150" width="11" height="28" rx="5" transform="rotate(22 175 1164)"/><rect x="440" y="1140" width="11" height="30" rx="5" transform="rotate(-20 445 1155)"/><rect x="720" y="1151" width="11" height="27" rx="5" transform="rotate(18 725 1164)"/></g>${close}`;
    return `${open}<g fill="none" stroke="${p.ink}" stroke-width="7" stroke-linecap="round" opacity=".56"><path d="M20 55 q55-42 110 0 t110 0"/><path d="M660 1145 q50 40 100 0 t120 0"/><path d="M735 12 l15 27 31 4-23 20 7 31-30-16-27 17 6-31-24-19 32-4z"/><circle cx="24" cy="1060" r="30"/><path d="M11 1060 h26 M24 1047 v26"/><path d="M870 400 q-30 48 0 96"/><path d="M27 635 q30-44 0-90"/></g>${close}`;
  }
  ...
  async function openCardDialog(){const dialog=$("#cardDialog"),box=$("#cardQrBox"),status=$("#cardQrStatus"),link=$("#cardShareLink");dialog.showModal();box.innerHTML="<span>しゃしんを つくっています…</span>";link.hidden=true;if(!state.name.trim()){status.textContent="なまえを いれると おくれるよ。";box.innerHTML="<span>なまえを いれてね</span>";return;}try{await drawCard();}catch(_){box.innerHTML="<span>画像をつくれませんでした</span>";status.textContent="もういちどためしてね。";return;}if(!transferInfo.enabled&&!(await checkTransferServer())){status.textContent="QR機能用サーバーに接続できません。このパソコンへの保存はできます。";box.innerHTML="<span>QR機能用サーバーが必要です</span>";return;}try{const dataUrl=$("#cardCanvas").toDataURL("image/png"),response=await fetch(appUrl("api/cards"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:dataUrl,filename:`${safeFilename(state.name)}-card.png`,title:`${state.name}のカード`})}),data=await response.json();if(!response.ok)throw new Error();const img=new Image();img.alt="カードをうけとるQRコード";img.onload=()=>box.replaceChildren(img);img.src=appUrl(`api/qr?text=${encodeURIComponent(data.cardUrl)}&t=${Date.now()}`);link.href=data.cardUrl;link.hidden=false;status.textContent="スマホで よみこんで、プレビューとおなじ写真を ほぞんしてね。";}catch(_){box.innerHTML="<span>QRをつくれませんでした</span>";status.textContent="このパソコンへの画像保存はできます。";}}
  ...
  async function checkTransferServer(){try{const response=await fetch(appUrl("api/config"),{cache:"no-store"});if(!response.ok)throw new Error();const data=await response.json();transferInfo={enabled:Boolean(data.enabled),baseUrl:String(data.baseUrl||"")};}catch(_){transferInfo={enabled:false,baseUrl:""};}return transferInfo.enabled;}async function ensureTransferServer(){if(transferInfo.enabled)return true;if(await checkTransferServer())return true;showToast("QR機能に接続できません。しばらくしてからもういちどためしてね");return false;}
